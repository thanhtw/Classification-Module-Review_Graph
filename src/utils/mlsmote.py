...
def nearest_neighbour(X):
    nbs = NearestNeighbors(n_neighbors=5, metric='cosine', algorithm='brute').fit(X)
    _, indices = nbs.kneighbors(X)
    return indices

def MLSMOTE(X, y, n_sample, random_state=42):
...
if __name__ == '__main__':
    ...
    y = pd.DataFrame(np.random.randint(0,2, (100,3)), columns=['label1','label2','label3'])
    X_sub, y_sub = get_minority_instace(X, y)
    X_res, y_res = MLSMOTE(X_sub, y_sub, n_sample=50, random_state=42)
